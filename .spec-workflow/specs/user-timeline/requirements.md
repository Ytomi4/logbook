...
**User Story:** ユーザーとして、まだログを書いていない本は本の情報だけを見たい。余計な「登録しました」というログは邪魔なので、必要なときだけ表示してほしい。

#### Acceptance Criteria

1. WHEN 本に登録ログしか存在しない（単体） THEN タイムラインは本の表紙のみを表示し、ログ部分は非表示にする SHALL
2. WHEN 本に登録ログ以外のログが存在する THEN タイムラインは登録ログも含めて全てのログを表示する SHALL
3. WHEN 複数のログをグループ表示する THEN 登録ログは他のログと同様に線と点で表示する SHALL
4. WHEN 登録ログのみの本を表示する THEN 本の表紙と基本情報（タイトル、著者）を表示する SHALL
